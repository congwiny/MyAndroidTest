...
import com.congwiny.camerahelper.invocation.TestInvocation;
...
//            m_MediaLiveHelper.setServerUrl(mLiveModel.getPublishUrl());
    ...
    private void addProxy() throws NoSuchFieldException, IllegalAccessException, NoSuchMethodException {
        ...
        SurfaceHolder mSurfaceHolder = m_SurfaceView.getHolder();
        // 创建代理接口的实现
        TestInvocation testInvocation = new TestInvocation(mSurfaceHolder);
        // 为 mSurfaceHolder 添加动态代理,并获取添加代理之后的 newSurfaceHolder
        SurfaceHolder newSurfaceHolder = (SurfaceHolder) Proxy.newProxyInstance(mSurfaceHolder.getClass().getClassLoader(),
                mSurfaceHolder.getClass().getInterfaces(),testInvocation);
        ...
        fieldHolder.set(m_SurfaceView,newSurfaceHolder);
    }
}
